...
export function buildWorkGraph(ownerId: string, input: { title: string; kind: WorkKind }, updatedAt: string): WorkGraph {
  ...
  const chapter: ServerChapter = { id: crypto.randomUUID(), workId: work.id, volumeId: volume.id, title: '第1章', canonicalContent: emptyCanonicalContent(), plainText: '', wordCount: 0, revision: 0, position: 0 };
