...
export function createWorkHandlers(dependencies: { requireUserId(request: Request): Promise<string>; assertMutation?(request: Request): void; store: WorkHandlerStore }) {
  return {
    ...
    async create(request: Request): Promise<Response> {
      try {
        ...
        const input = await request.json() as { title?: string; kind?: WorkKind };
