...
describe('password hashing', () => {
  it('stores PBKDF2 metadata and never the password', async () => {
    const stored = await hashPassword('not-stored');
    ...
    expect(JSON.stringify(stored)).not.toContain('not-stored');
    await expect(verifyPassword('not-stored', stored)).resolves.toBe(true);
  });
});
