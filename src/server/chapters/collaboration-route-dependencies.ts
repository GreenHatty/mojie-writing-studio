...
export function collaborationHandlersFromRuntime() {
  ...
  return createCollaborationHandlers({ store: createD1CollaborationStore(bindings.DB), async requireUserId(request) { const token = readCookie(request.headers.get('Cookie'), names.session); if (!token) throw new AppError('UNAUTHENTICATED', 401); return (await requireActiveSession(sessions, token)).userId; }, assertMutation(request) { assertCsrf({ origin: request.headers.get('Origin'), expectedOrigin: bindings.APP_ORIGIN, cookieToken: readCookie(request.headers.get('Cookie'), names.csrf), headerToken: request.headers.get('X-CSRF-Token') }); } });
}
