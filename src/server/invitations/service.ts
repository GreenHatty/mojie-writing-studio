...
export type InvitationRole = PlatformRole | WorkRole;
export type InvitationRecord = { id: string; tokenHash: string; createdBy: string; role: InvitationRole; workId: string | null; expiresAt: string; maxUses: number; useCount: number; revokedAt: string | null };
...
export async function hashInvitationToken(token: string): Promise<string> { return encode(new Uint8Array(await crypto.subtle.digest('SHA-256', new TextEncoder().encode(token)))); }
...
export function createInvitationService(store: InvitationStore) {
  return {
    async create(input: { createdBy: string; role: InvitationRole; workId: string | null; expiresAt: string; maxUses: number }) {
      ...
      const record: InvitationRecord = { id: crypto.randomUUID(), tokenHash: await hashInvitationToken(token), ...input, useCount: 0, revokedAt: null };
      await store.put(record);
      return { id: record.id, token };
    },
    async accept(token: string, now = new Date()): Promise<InvitationRecord> {
      const record = await store.findByHash(await hashInvitationToken(token));
