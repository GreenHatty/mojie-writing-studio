...
export function ServerEditor({ chapterId, csrf, userId, draftDek, onBack }: { chapterId: string; csrf: string; userId: string; draftDek: Uint8Array; onBack(): void }) {
  ...
  async function createWorkInvitation() {
    if (!directory) return; const selected = window.prompt('邀请权限：EDITOR、COMMENTER 或 VIEWER', 'COMMENTER')?.toUpperCase();
    if (selected !== 'EDITOR' && selected !== 'COMMENTER' && selected !== 'VIEWER') return;
    const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60_000).toISOString();
    const response = await fetch('/api/invitations', { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': csrf }, body: JSON.stringify({ role: selected, workId: directory.work.id, expiresAt, maxUses: 1 }) });
    if (!response.ok) { setState('error'); return; } const { token } = await response.json() as { token: string }; setInvitationLink(`${window.location.origin}/invitations/${encodeURIComponent(token)}`); setRightTab('collaboration'); setRightOpen(true);
  }
