...
export type WorldMapCanvasPreset = 'standard' | 'wide' | 'panorama' | 'tall';
export type WorldMapCanvasSettings = { preset: WorldMapCanvasPreset; widthPercent: number; backgroundScale: number };

export const WORLD_MAP_CANVAS_PRESETS: ReadonlyArray<{ id: WorldMapCanvasPreset; label: string; ratio: string; widthPercent: number; help: string }> = [
  { id: 'standard', label: '原图幅面', ratio: '3 / 2', widthPercent: 100, help: '保持底图原始三比二比例' },
  { id: 'wide', label: '横向拓界', ratio: '2 / 1', widthPercent: 130, help: '向左右扩充，适合多国并列与远征路线' },
  { id: 'panorama', label: '大陆全景', ratio: '5 / 2', widthPercent: 165, help: '大幅扩展东西边界，适合群洲与诸国世界' },
  { id: 'tall', label: '纵向拓界', ratio: '4 / 3', widthPercent: 110, help: '扩充南北边界，适合纵深地貌与层级世界' }
] as const;
...
export function WorldPlateMapWorkbench({
  items, backgroundId, canvasSettings, readOnly, onBackgroundChange, onCanvasChange, onCreate, onUpdate, onDelete, onDeleteMany, onMerge
}: {
  items: WorldMapItem[];
  backgroundId: string;
  canvasSettings?: WorldMapCanvasSettings;
  readOnly?: boolean;
  onBackgroundChange(id: string): Promise<void>;
  onCanvasChange(settings: WorldMapCanvasSettings): Promise<void>;
  ...
  const appliedCanvas = canvasSettings ?? { preset: 'standard' as const, widthPercent: 100, backgroundScale: 100 };
  const [canvasDraft, setCanvasDraft] = useState<WorldMapCanvasSettings>(appliedCanvas);
  const canvasPreset = WORLD_MAP_CANVAS_PRESETS.find((item) => item.id === canvasDraft.preset) ?? WORLD_MAP_CANVAS_PRESETS[0];
  ...
  useEffect(() => {
    setCanvasDraft(appliedCanvas);
  }, [appliedCanvas.backgroundScale, appliedCanvas.preset, appliedCanvas.widthPercent]);
  ...
  return <section className="world-plate-workbench">
    ...
    <div className="map-canvas-expansion" aria-label="底图边界与画布幅面">
      <div className="map-canvas-heading"><div><strong>底图边界与宽幅</strong><span>原底图保持比例居中，扩出的牛皮纸边界可继续放置地形、城市和地名。</span></div><HelpTip text="幅面决定地图比例；工作区宽度决定编辑时横向可滚动的实际空间；底图占比越小，四周可继续创作的留白越多。" /></div>
      <div className="map-canvas-presets" role="group" aria-label="地图幅面预设">{WORLD_MAP_CANVAS_PRESETS.map((preset) => <button aria-pressed={canvasDraft.preset === preset.id} disabled={busy || readOnly} key={preset.id} onClick={() => setCanvasDraft((current) => ({ ...current, preset: preset.id, widthPercent: preset.widthPercent }))} title={preset.help} type="button"><b>{preset.label}</b><span>{preset.ratio.replace(' / ', ':')}</span></button>)}</div>
      <div className="map-canvas-sliders">
        <label><span>工作区宽度 {canvasDraft.widthPercent}%</span><input disabled={busy || readOnly} max="200" min="100" onChange={(event) => setCanvasDraft((current) => ({ ...current, widthPercent: Number(event.target.value) }))} step="5" type="range" value={canvasDraft.widthPercent} /></label>
        <label><span>原底图占画布 {canvasDraft.backgroundScale}%</span><input disabled={busy || readOnly} max="100" min="55" onChange={(event) => setCanvasDraft((current) => ({ ...current, backgroundScale: Number(event.target.value) }))} step="5" type="range" value={canvasDraft.backgroundScale} /></label>
        <button disabled={busy || readOnly || (canvasDraft.preset === appliedCanvas.preset && canvasDraft.widthPercent === appliedCanvas.widthPercent && canvasDraft.backgroundScale === appliedCanvas.backgroundScale)} onClick={() => void safe(() => onCanvasChange(canvasDraft))} type="button">应用画布</button>
      </div>
    </div>
    ...
    <div className="world-plate-viewport"><div
      aria-label="可绘制世界地图"
      ...
      role="application"
      style={{
        '--world-map-background': `url("${sample.src}")`,
        '--world-map-aspect': canvasPreset.ratio,
        '--world-map-width': `${canvasDraft.widthPercent}%`,
        '--world-map-background-size': canvasDraft.preset === 'wide' || canvasDraft.preset === 'panorama' ? `auto ${canvasDraft.backgroundScale}%` : `${canvasDraft.backgroundScale}% auto`
      } as CSSProperties}
    >
