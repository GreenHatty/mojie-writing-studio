...
export const WORLD_PLATE_SAMPLES = [
  { id: 'modern-city', label: '现代都市圈', src: '/map-assets/modern-city.webp', help: '环形都市、港口群岛、卫星城与山海交通板块' },
  { id: 'cultivation-continent', label: '修仙大陆', src: '/map-assets/cultivation-continent.webp', help: '五域大陆、仙海、禁区与宗门势力板块' },
  { id: 'supernatural-apocalypse', label: '异能末世', src: '/map-assets/supernatural-apocalypse.webp', help: '灾变隔离区、幸存城市、感染带与异能势力板块' },
  { id: 'wasteland-world', label: '废土世界', src: '/map-assets/wasteland-world.webp', help: '裂谷荒漠、辐射盆地、绿洲据点与旧文明板块' },
  { id: 'wuxia-realms', label: '古代武侠', src: '/map-assets/wuxia-realms.webp', help: '中原州府、边关、江湖门派与水路商道板块' },
  { id: 'primordial-world', label: '洪荒世界', src: '/map-assets/primordial-world.webp', help: '天地四极、神族祖地、混沌海与先天禁区板块' },
  { id: 'western-fantasy', label: '西幻世界', src: '/map-assets/western-fantasy.webp', help: '王国、公国、精灵森林、龙脉与外海群岛板块' }
] as const;
...
export function WorldPlateMapWorkbench({
  items, backgroundId, readOnly, onBackgroundChange, onCreate, onUpdate, onDelete, onMerge
}: {
  items: WorldMapItem[];
  backgroundId: string;
  readOnly?: boolean;
  ...
  async function finishDrawing() {
    ...
    await safe(() => onCreate({ title, markerType: tool, x: first.x, y: first.y, path, scale: 1, rotation: 0 }));
    setDraft([]);
  }
  ...
  return <section className="world-plate-workbench">
    ...
    <div className="world-plate-samples" aria-label="题材世界底图">{WORLD_PLATE_SAMPLES.map((item) => <button aria-pressed={sample.id === item.id} disabled={busy || readOnly} key={item.id} onClick={() => void safe(() => onBackgroundChange(item.id))} title={item.help} type="button"><img alt="" src={item.src} /><span>{item.label}</span></button>)}</div>
    <div className="world-map-controls">
      <div className="world-map-tools">{(Object.keys(LABELS) as WorldMapTool[]).map((type) => <button aria-pressed={tool === type} className={`tool-${type}`} disabled={readOnly || busy} key={type} onClick={() => setTool(type)} title={type === 'eraser' ? '点击任一贴图或笔画将其移入回收站，可用撤回恢复' : `${LABELS[type]}工具`} type="button"><span className={`map-tool-swatch sticker-${type}`} aria-hidden="true" /><b>{LABELS[type]}</b></button>)}</div>
    ...
    <div
      ...
      onPointerDown={(event) => { if (readOnly || busy || event.target !== event.currentTarget && (event.target as HTMLElement).closest('[data-map-item]')) return; event.currentTarget.setPointerCapture(event.pointerId); setDraft([boardPoint(event, event.currentTarget)]); }}
      onPointerMove={(event) => { if (!event.currentTarget.hasPointerCapture(event.pointerId) || !PATH_TOOLS.has(tool)) return; const next = boardPoint(event, event.currentTarget); setDraft((current) => { const last = current.at(-1); return !last || Math.hypot(next.x - last.x, next.y - last.y) > 1.7 ? [...current, next] : current; }); }}
      onPointerUp={() => void finishDrawing()}
      ref={boardRef}
      role="application"
      style={{ '--world-map-background': `url("${sample.src}")` } as CSSProperties}
    >
      <svg className="world-map-paths" preserveAspectRatio="none" viewBox="0 0 100 100">{[...paths.map((item) => ({ id: item.id, markerType: item.markerType, points: item.points, draft: false })), ...(draft.length > 1 ? [{ id: 'draft', markerType: tool, points: draft.map((point) => [point.x, point.y] as [number, number]), draft: true }] : [])].map((path) => <polyline className={`world-map-line is-${path.markerType} ${path.draft ? 'is-draft' : ''}`} data-map-item={path.draft ? undefined : path.id} key={path.id} onClick={(event) => { event.stopPropagation(); if (!path.draft && tool === 'eraser') void safe(() => onDelete(path.id)); }} points={path.points.map(([x, y]) => `${x},${y}`).join(' ')} />)}</svg>
      {[...paths.map((item) => ({ id: item.id, markerType: item.markerType, points: item.points })), ...(draft.length > 1 ? [{ id: 'draft', markerType: tool, points: draft.map((point) => [point.x, point.y] as [number, number]) }] : [])].filter((path) => ['mountain', 'forest', 'river', 'route'].includes(path.markerType)).flatMap((path) => path.points.filter((_, index) => index % (path.markerType === 'river' || path.markerType === 'route' ? 3 : 2) === 0).map(([x, y], index) => <span aria-hidden="true" className={`world-map-sticker sticker-${path.markerType}`} key={`${path.id}:${index}`} style={{ left: `${x}%`, top: `${y}%` }} />))}
      {items.filter((item) => !item.path?.length).map((item) => <button
        className={`world-map-item ${item.markerType === 'label' ? 'is-label' : 'world-map-sticker sticker-' + item.markerType} ${selectedId === item.id ? 'is-selected' : ''}`}
        data-map-item={item.id}
        key={item.id}
        onClick={(event) => { event.stopPropagation(); if (tool === 'eraser') void safe(() => onDelete(item.id)); else setSelectedId(item.id); }}
