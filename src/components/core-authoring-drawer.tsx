  ...
  return <div aria-label="写作工具箱" aria-modal="true" className="authoring-drawer-backdrop" role="dialog">
    <section className="authoring-drawer">
      ...
      <nav aria-label="工具箱栏目">
        {([['templates', '模板', '按平台、读者、篇幅和题材选择结构模板，并查看逐步成功示例'], ['planning', '策划卡', '把模板变成当前作品可编辑的故事策划，不自动生成正文'], ['checks', '检查与工具', '对当前章节做非破坏性检查、取名、快捷短语与计算'], ['lessons', '写作课堂', '用操作步骤、改写对照和核心设定优化器解决具体写作问题'], ['files', '文件与备份', '导入导出项目、正文与 DOCX，并管理本地或外部备份']] as Array<[DrawerTab, string, string]>).map(([id, label, help]) => <button aria-current={tab === id ? 'page' : undefined} key={id} onClick={() => setTab(id)} title={help} type="button">{label}</button>)}
      </nav>
      <div className="authoring-drawer-body">
        {tab === 'templates' ? <TemplateLibrary onUseTemplate={(template) => void useTemplate(template)} /> : null}
        {tab === 'planning' ? planningCard ? <section className="planning-card-editor"><div className="panel-section-heading"><div><p className="eyebrow">{planningCard.templateName}</p><h2>可编辑策划卡</h2></div><button onClick={() => void savePlanning()} title="保存到当前账号的本机加密草稿空间" type="button">保存策划卡</button></div><p>这些字段是大纲骨架，不会自动写入正文。</p>{planningCard.sections.map((section, index) => <div className="planning-card-field" key={section.key}><label><span className="field-label-with-actions"><span>{section.title}</span><HelpTip text={`${section.prompt}。可从本地 TXT、Markdown、HTML 或 DOCX 追加/替换。`} /></span><small>{section.prompt}</small><textarea onBlur={() => void savePlanning()} onChange={(event) => setPlanningCard((current) => current ? { ...current, sections: current.sections.map((item, currentIndex) => currentIndex === index ? { ...item, value: event.target.value } : item) } : current)} placeholder="填写你的设定" value={section.value} /></label><LocalContentImporter compact label={`导入${section.title}`} onApply={(text, mode) => setPlanningCard((current) => current ? { ...current, sections: current.sections.map((item, currentIndex) => currentIndex === index ? { ...item, value: mode === 'append' && item.value.trim() ? `${item.value.replace(/\s+$/u, '')}\n\n${text}` : text } : item) } : current)} /></div>)}</section> : <div className="context-empty">先从“模板”选择一份结构，系统会建立可编辑策划卡，不生成完整正文。</div> : null}
        ...
        {tab === 'lessons' ? <LessonsPanel csrf={csrf} directory={directory} /> : null}
